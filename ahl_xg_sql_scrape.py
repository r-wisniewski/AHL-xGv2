    ...
    try:
        ...
        for event_obj in json_data:
            try:
                ...
                #get the shooter's name and the goalie's name
                ...
                if event == "goal":
                    ...
                    team_abv = str(team_id_dict[team_id])
                elif event == "shot":
                    team_id = int(safe_get(event_obj, 'details', 'shooterTeamId'))
                    team_abv = str(team_id_dict[team_id])

                if not team_abv:
                    log.debug(f"Game {game_id}: {event} team ID {team_id} not found in dictionary")
                    continue
                ...
                xLocation = int(x_loc)
                if xLocation > 300: #if the goal is for the "other" team
                ...
                values = """INSERT INTO ahlxgfv2 (GameID, TeamID, XLocation, YLocation, Goal, Strength, Period, Shooter, Goalie) 
                           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)"""
                
                mutex.acquire()
                try:
                    cursor.execute(values, (game_id, team_abv, xLocation, yLocation, is_goal, event_strength, period, shooter_name, goalie_name))
                    connection.commit()
                    shots_processed += 1
                finally:
                    mutex.release()
